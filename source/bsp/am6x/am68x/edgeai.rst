...
On Linux, the most convenient way to install the BSP to an SD card is by using bmap-tools.

.. code-block:: console

      host$ sudo apt install bmap-tools

Then the wic image and its bmap file can be downloaded from
https://download.phytec.de/Software/Linux/BSP-Yocto-EdgeAI/BSP-Yocto-EdgeAI-AM68x-v0.1/

.. code-block:: console

      host$ wget https://download.phytec.de/Software/Linux/BSP-Yocto-EdgeAI/BSP-Yocto-EdgeAI-AM68x-v0.1/images/ampliphy-xwayland/phyboard-izar-am68x-2/phytec-edgeai-image-phyboard-izar-am68x-2.wic.xz
      host$ wget https://download.phytec.de/Software/Linux/BSP-Yocto-EdgeAI/BSP-Yocto-EdgeAI-AM68x-v0.1/images/ampliphy-xwayland/phyboard-izar-am68x-2/phytec-edgeai-image-phyboard-izar-am68x-2.wic.bmap

And then, the image can be flashed onto an SD card:

.. code-block:: console

      host$ sudo bmaptool copy phytec-edgeai-image-phyboard-izar-am68x-2.wic.xz /dev/sdX

Configuring the phyBOARD-Izar
.............................

Configure the phyBOARD-Izar to boot from the SD card by setting the dip
switches S8 and S7:

.. figure:: images/SD_Card_Boot.png

By default, the phyboard-izar-am68x-2 image will enable both the devicetree
overlays for the 10.1'' ETML1010G3DRA LVDS display and the phyCAM-M VM-016 on
CSI0 (X54), which come with the development kit.

If you need to change the default overlays, you can do so by editing
the bootenv.txt file in the SD card's fat partition.

.. code-block:: console

      root@phyboard-izar-am68x-2:~# cat /boot/bootenv.txt
      overlays=k3-am68-phyboard-izar-lvds-ac200.dtbo k3-am68-phyboard-izar-pwm-fan.dtbo  k3-am68-phyboard-izar-vm016-mipi-csi0.dtbo k3-j721s2-edgeai-apps.dtbo

The phyboard-izar-am68x-3 image comes with an empty bootenv.txt file.

Running the Demos
-----------------

In addition to the demos included in the SDK Linux Edge AI from TI, there are
a few Gstreamer pipelines located in the rootfs usingthe VM-016 phyCAM-M CSI
Camera as a source.
...
This pipeline is supposed to be used on a Linux host PC to display an incoming
RTP stream using eth0:
