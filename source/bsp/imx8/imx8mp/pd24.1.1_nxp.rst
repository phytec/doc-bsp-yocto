...
.. |mcore-sdk-version| replace:: v2.13.0-phy
...
<sdk-directory>/examples-phytec/phyboard-pollux/<example_category>/<example>/armgcc.
There are scripts for each memory location the firmware is supposed to run in,
e.g.

.. code-block:: console

   host:~$ cd examples-phytec/phyboard-pollux/demo_apps/hello_world/armgcc
...
Restart the target and add to/set the optargs variable::

   u-boot=> env set optargs "$optargs clk-imx8mp.mcore_booted"
