...
.. code-block:: console
   ...
   host:~$ sudo apt install tftpd-hpa nfs-kernel-server isc-dhcp-server
...
*  Create or edit the ``/etc/dhcp/dhcpd.conf`` file; Using the internal subnet
   sample:
   ...
   .. code-block::
         ...
         subnet 192.168.3.0 netmask 255.255.255.224 {
           ...
           range 192.168.3.11 192.168.3.30;
           option domain-name-servers ns1.internal.devboard;
           option domain-name "internal.devboard";
...
.. warning::
   ...
   that via the ``interface`` configuration option.


*  Now the DHCP-Server has to read the ``/etc/dhcp/dhcpd.conf`` file again:

   .. code-block:: console

         host:~# service isc-dhcp-server restart
