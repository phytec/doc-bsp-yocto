...
codec on the PEB-AV-10 expansion board. On the PEB-AV-10 there is a 3.5mm
headset jack with OMTP-standard and an 8-pin header. The 8-pin header contains
a mono speaker, headphones, and line in signals.

.. note::

   Using the PEB-AV-10 expansion board for display output along with HDMI as
   audio output is not supported. The audio output device must match the video
   output device.
