...
.. |ref-supported-hardware| replace:: :ref:`Supported Hardware <imx8mp-pd22.1.2-supported-hardware>`
...
.. |ref-build-uboot| replace:: :ref:`Build U-Boot <imx8mp-pd22.1.2-development-build-uboot>`
...
==================== ================ ================ ================= ==========
|yocto-manifestname| Minor            |yocto-codename| 2024/08/05        Released
==================== ================ ================ ================= ==========

.. include:: ../../intro.rsti

.. _imx8mp-pd22.1.2-supported-hardware:
...
.. _imx8mp-pd22.1.2-development-build-uboot:
...
To build the bootloader, you need to **copy** these **files** to your
|u-boot-repo-name| **build directory** and rename them to fit with *mkimage*
script:
...
===== ======================= ================================= ============
|soc| |bootloader-offset| kiB |bootloader-offset-boot-part| kiB /dev/mmcblk2
===== ======================= ================================= ============
...
.. code-block:: console
   ...
   host:~$ sudo dd if=flash.bin of=/dev/sd[x] bs=1024 seek=|bootloader-offset| conv=fsync
...
.. code-block:: kconfig
   ...
   # CONFIG_|u-boot-socname-config|_RAM_SIZE_4GB_2GHZ=y
...
*  *Carrierboard.dtsi* - Devices that come from the |soc| SoC but are just
   routed down to the carrier board and used there are included in this dtsi.
*  *Board.dts* - include the carrier board and module dtsi files. There may
   ...
   board configuration). For example, there are |som| SOMs which may or may not
...
*  Now, you can set enhanced attribute for the whole device, e.g. 3719168 KiB,
   by typing:
...
===== ======================= =========== ==============
SoC   Offset User Area        Offset Boot eMMC Device
                              Partition
===== ======================= =========== ==============
...
.. include:: /bsp/imx-common/resize-rootfs.rsti
...
this could be for example ``imx8_phytec_distro.config``::
...
`Yocto Reference Manual <yocto-ref-manual-kernel-and-bootloader-config_>`_.
...
the ``max_brightness`` file. The brightness file will set the brightness of the
LED (taking a value from 0 up to max_brightness). Most LEDs do not have
hardware brightness support and will just be turned on by all non-zero
brightness settings.
...
option for the driver, which must be enabled, is named ``CONFIG_IWLWIFI`` and
can be found under
*Intel Wireless WiFi Next Gen AGN - Wireless-N/Advanced-N/Ultimat* in
...
toggle by hitting '**<**' for left and '**>**' for right output. With the
**tab** key, you can switch between controls for playback and recording.
...
If the sound is not audible change playback devices to the software volume
control playback devices, set *playback.pcm* to the respective softvol playback
device either "softvol_hdmi" or "softvol_pebav10". Use alsamixer controls to
...
Line In as the default input source. To select a different audio source you can
use ``alsamixer``. For example, switch on *Right PGA Mixer Mic3R* and *Left PGA
Mixer Mic3R* in order to capture the audio from the microphone input of the
...
https://community.nxp.com/t5/NXP-Tech-Blog/PyeIQ-3-x-Release-User-Guide/ba-p/1305998.
...
Reading the registers using ``/dev/mem`` will cause the system to hang unless
the *ocotp_root_clk* is enabled. To enable this clock permanent, add to the
device tree:
